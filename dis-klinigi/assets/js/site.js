/* Mine Ağız ve Diş Sağlığı — shared site behaviour (all pages) */
(function () {
  'use strict';

  var cfg = window.MINE_CONFIG || {};
  ...
  var S = {
    ...
    'demo.label': 'Demo',
    'demo.text': 'Bu bir demo görünümüdür, sizin için özelleştirilecektir. Klinik adı, ekip, fotoğraflar, adres ve bilgiler örnek olarak kullanıldı.',
    'demo.close': 'Tamam',
    ...
    'day.0': 'Pazar', 'day.1': 'Pazartesi', 'day.2': 'Salı', 'day.3': 'Çarşamba',
    'day.4': 'Perşembe', 'day.5': 'Cuma', 'day.6': 'Cumartesi'
  };
  function t(key, vars) {
    var str = S[key] != null ? S[key] : key;
  ...
  function renderConfigText() {
    ...
    $$('[data-address]').forEach(function (el) { if (cfg.address) el.textContent = cfg.address; });
  }
  ...
  function applyTheme(theme) {
    ...
    if (themeMeta) themeMeta.setAttribute('content', theme === 'dark' ? '#050505' : '#F5F5F7');
    store('mine-theme', theme);
    updateThemeLabel();
  ...
  function buildFaqSchema() {
    ...
    node.textContent = JSON.stringify({ '@context': 'https://schema.org', '@type': 'FAQPage', inLanguage: 'tr-TR', mainEntity: items });
  }
  ...
  function showDemoNotice() {
    ...
    try { if (sessionStorage.getItem('mine-demo-seen')) return; } catch (e) {}
    ...
    ok.addEventListener('click', function () {
      try { sessionStorage.setItem('mine-demo-seen', '1'); } catch (e) {}
      bar.remove();
