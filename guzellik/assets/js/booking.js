/* Neva Güzellik — online appointment booking */
(function () {
  'use strict';
  ...
  var STR = {
    tr: {
      type: { analysis: 'Cilt analizi', single: 'Tek seans', package: 'Paket seansı' },
      noFee: 'Ücretsiz',
      clinic: 'Merkezde',
      ...
      noteDay: 'Seans süresi {dur} dakika. Saatler Türkiye saatidir.',
      empty: 'Bu gün için uygun saat kalmadı.',
      emptyNext: 'Sonraki uygun güne geç',
      noDays: 'Önümüzdeki {n} gün içinde bu seçimle uygun saat bulunamadı. Uzman veya seans türünü değiştirmeyi deneyin ya da bizi arayın.',
      ...
      notForArea: 'Bu uygulamayı yapmıyor',
      needStaff: 'Seçtiğiniz uzman bu uygulamayı yapmıyor. Başka bir uzman seçin.',
      ...
      sum: { type: 'Seans', area: 'Uygulama', staff: 'Uzman', date: 'Tarih', time: 'Saat', place: 'Yer', none: 'Seçilmedi' },
      rev: { appointment: 'Seans', datetime: 'Tarih ve saat', name: 'Ad soyad', phone: 'Telefon', email: 'E-posta', note: 'Not', reminder: 'Hatırlatma', yes: 'E-postayla bir gün önce', no: 'İstemiyor', edit: 'Düzenle' },
      prepTitle: 'Seanstan önce',
      prep: ['Randevu saatinden 10 dakika önce gelmeniz yeterli.', 'Cildinizi makyajsız getirin; gerekirse temizliği merkezde yapıyoruz.', 'Lazer seansı öncesinde son 4 hafta güneşlenmemiş, son 2 hafta ağda yapmamış olmanız gerekiyor — bölgeyi jiletle almanız yeterli.', 'Kullandığınız ilaç, asit içerikli krem veya takviyeler varsa randevu sırasında bize söyleyin.'],
      submitErr: {
      ...
      demoText: 'Bu bir demo olduğu için gerçek bir randevu oluşturulmadı ve e-posta gönderilmedi. Merkezinize kurulan sürümde randevu bu adımda takviminize işlenir, müşteriye de onay e-postası gider.',
      waText: 'Merhaba, {date} saat {time} için {type} randevusu oluşturdum.\nReferans: {ref}\nAd soyad: {name}',
      waFallback: 'Merhaba, randevu almak istiyorum.\nSeans: {type}\nUzman: {staff}\nTarih: {date} {time}\nAd soyad: {name}\nTelefon: {phone}\nE-posta: {email}',
      icsSummary: 'Neva Güzellik · {type}',
      ...
      cancel: {
        refErr: 'Referans kodunu NV-XXXXX biçiminde yazın.',
    ...
    en: {
      type: { analysis: 'Skin analysis', single: 'Single session', package: 'Package session' },
      noFee: 'Free',
      clinic: 'At the studio',
      ...
      noDays: 'No available times in the next {n} days for this selection. Try another specialist or session type, or give us a call.',
      ...
      notForArea: 'Doesn’t do this treatment',
      needStaff: 'The specialist you picked doesn’t do this treatment. Please choose someone else.',
      ...
      sum: { type: 'Session', area: 'Treatment', staff: 'Specialist', date: 'Date', time: 'Time', place: 'Place', none: 'Not selected' },
      rev: { appointment: 'Session', datetime: 'Date and time', name: 'Full name', phone: 'Phone', email: 'Email', note: 'Note', reminder: 'Reminder', yes: 'By email, the day before', no: 'No reminder', edit: 'Edit' },
      prepTitle: 'Before your appointment',
      prep: ['Arriving 10 minutes early is enough.', 'Come without make-up; we can cleanse your skin here if needed.', 'Before a laser session: no sun exposure for 4 weeks and no waxing for 2 weeks — shaving the area is all that is required.', 'Tell us about any medication, acid-based creams or supplements you are using.'],
      submitErr: {
      ...
      icsSummary: 'Neva Beauty · {type}',
      icsDesc: 'Specialist: {staff}\nReference: {ref}\nTo change: {phone}',
      ...
      cancel: {
        refErr: 'Enter the reference code as NV-XXXXX.',
  ...
  function fmtPrice(n) { return n ? new Intl.NumberFormat(locale()).format(n) + ' TL' : S().noFee; }
  ...
  /* ---------- Local demo storage ---------- */
  var DEMO_KEY = 'neva-demo-bookings';
  function demoList() {
  ...
  var state = {
    step: 1, maxStep: 1,
    type: 'analysis', area: 'skin', staff: 'any',
  ...
  function renderNextSlot() {
    ...
    var sel = { type: 'analysis', area: null, staff: 'any' };
    var now = T.now();
  ...
  document.addEventListener('click', function (e) {
    ...
    if (t.hasAttribute('data-next-book')) {
      preselect({ type: 'analysis', staff: t.dataset.staff, date: t.dataset.date, time: t.dataset.time });
      return;
    }
    preselect({ area: t.getAttribute('data-book-area'), type: t.getAttribute('data-book-type'), staff: t.getAttribute('data-book-staff') });
  });
  document.addEventListener('neva:book', function (e) {
    preselect(e.detail || {});
    var rect = bookingEl.getBoundingClientRect();
    window.scrollTo({ top: window.scrollY + rect.top - 96, behavior: M.reduceMotion ? 'auto' : 'smooth' });
  });
  ...
  function placeLabel() {
    return cfg.address[M.lang()] || cfg.address.tr;
  }
  ...
  function renderReview() {
    ...
    var ul = el('ul');
    s.prep.forEach(function (p) { ul.appendChild(el('li', null, p)); });
  ...
  function buildIcs(r) {
    ...
    var place = cfg.address[M.lang()] || cfg.address.tr;
    var lines = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Neva Güzellik//Randevu//TR', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
      'BEGIN:VEVENT',
      'UID:' + r.ref + '@nevaguzellik',
  ...
  function renderDone() {
    ...
    row(dl, 'review-row', s.sum.place, cfg.address[M.lang()] || cfg.address.tr);
    ...
    ics.addEventListener('click', function () {
      ...
      a.href = url; a.download = 'neva-randevu-' + r.ref + '.ics';
  ...
  if (cancelForm) {
    ...
    cancelForm.addEventListener('submit', function (e) {
      ...
      if (/^NV[A-Z0-9]{5}$/.test(ref)) ref = 'NV-' + ref.slice(2);
      var email = cancelForm.elements.email.value.trim();
      var refOk = /^NV-[A-Z0-9]{5}$/.test(ref);
