/**
 * Neva Güzellik — Google Apps Script sunucusu
 *
 ...
 *
 * ÖNEMLİ: Uzman, seans türü veya çalışma saatlerini değiştirirseniz sitedeki
 * assets/js/config.js dosyasını da aynı şekilde güncelleyin.
 */

var CONFIG = {
  CLINIC_NAME: 'Neva Güzellik ve Cilt Bakım Merkezi',
  ...
  MIN_NOTICE_MIN: 90,                  // En erken: şu an + 90 dk
  NOTICE_TOLERANCE_MIN: 5,             // Bilgisayar saati kayması için tolerans
  WINDOW_DAYS: 45,                     // Kaç gün sonrasına kadar randevu alınabilir
  STEP_MIN: 30,
...
var TYPES = {
  analysis: { minutes: 30, price: 0,    tr: 'Cilt analizi',  en: 'Skin analysis' },
  single:   { minutes: 60, price: 2400, tr: 'Tek seans',     en: 'Single session' },
  package:  { minutes: 45, price: 1750, tr: 'Paket seansı',  en: 'Package session' }
};

var AREAS = {
  skin:   { tr: 'Cilt bakımı', en: 'Facial' },
  laser:  { tr: 'Lazer epilasyon', en: 'Laser hair removal' },
  spot:   { tr: 'Leke ve akne', en: 'Pigmentation & acne' },
  body:   { tr: 'Vücut bakımı', en: 'Body treatment' },
  brow:   { tr: 'Kaş ve kirpik', en: 'Brows & lashes' },
  bridal: { tr: 'Gelin bakımı', en: 'Bridal care' }
};

/* 0 = Pazar, 1 = Pazartesi … 6 = Cumartesi */
var STAFF = {
  derya: {
    name: 'Derya Aydın', short: 'Derya',
    areas: ['skin', 'spot', 'bridal', 'body'],
    hours: { 1: [['10:00', '14:00'], ['15:00', '19:00']], 2: [['10:00', '14:00'], ['15:00', '19:00']], 3: [['10:00', '14:00'], ['15:00', '19:00']], 4: [['10:00', '14:00'], ['15:00', '19:00']], 5: [['10:00', '14:00'], ['15:00', '18:00']] }
  },
  pelin: {
    name: 'Pelin Sarıkaya', short: 'Pelin',
    areas: ['laser', 'body', 'skin'],
    hours: { 2: [['12:00', '16:00'], ['16:30', '20:00']], 3: [['12:00', '16:00'], ['16:30', '20:00']], 4: [['12:00', '16:00'], ['16:30', '20:00']], 5: [['12:00', '16:00'], ['16:30', '20:00']], 6: [['10:00', '17:00']] }
  },
  ece: {
    name: 'Ece Demirtaş', short: 'Ece',
    areas: ['brow', 'skin', 'bridal'],
    hours: { 1: [['11:00', '15:00'], ['16:00', '20:00']], 3: [['11:00', '15:00'], ['16:00', '20:00']], 5: [['11:00', '15:00'], ['16:00', '20:00']], 6: [['10:00', '16:00']] }
  }
};
...
var SHEETS = {
  bookings: {
    name: 'Randevular',
    headers: ['Oluşturulma', 'Referans', 'Durum', 'Tarih', 'Saat', 'Bitiş', 'Süre (dk)', 'Seans', 'Uygulama', 'Uzman', 'Uzman Kodu', 'Ad Soyad', 'Telefon', 'E-posta', 'Not', 'Hatırlatma', 'Dil', 'Takvim Etkinliği', 'İptal Zamanı', 'Hatırlatma Gönderildi'],
    widths: [150, 90, 120, 95, 60, 60, 70, 150, 140, 170, 90, 170, 130, 210, 280, 90, 50, 120, 150, 150]
  },
  closures: {
    name: 'Kapalı Günler',
    headers: ['Tarih', 'Uzman (boş = tüm merkez)', 'Başlangıç (boş = tüm gün)', 'Bitiş', 'Açıklama'],
...
function cancel_(data) {
  ...
  if (!/^NV-[A-Z0-9]{5}$/.test(ref) || !EMAIL_RE.test(email)) return { ok: false, error: 'not_found' };
...
function sendReminders() {
  ...
  readBookings_().forEach(function (b) {
    ...
    var t = MAIL[lang];
    var type = TYPES[typeKeyFromLabel_(b.typeLabel)] || TYPES.analysis;
    try {
      MailApp.sendEmail({
        to: b.email,
        ...
        body: fill_(t.reminderBody, {
          ...
          staff: STAFF[b.staff] ? STAFF[b.staff].name : '', place: CONFIG.CLINIC_ADDRESS[lang],
          ref: b.ref, phone: CONFIG.CLINIC_PHONE, clinic: CONFIG.CLINIC_NAME, prep: t.prep
        }),
        name: CONFIG.CLINIC_NAME,
        replyTo: clinicEmail_()
      });
...
var MAIL = {
  tr: {
    ...
    messageSubject: 'Mesajınızı aldık · Neva Güzellik',
  ...
  en: {
    ...
    messageSubject: 'We received your message · Neva Beauty',
...
function confirmClientBooking_(d, staffId, ref) {
  ...
  var vars = {
    ...
    place: CONFIG.CLINIC_ADDRESS[d.lang],
    price: formatPrice_(type.price, d.lang), prep: t.prep,
    cancelHours: CONFIG.CANCEL_NOTICE_HOURS, site: CONFIG.SITE_URL, phone: CONFIG.CLINIC_PHONE, clinic: CONFIG.CLINIC_NAME
  };
  var ics = Utilities.newBlob(buildIcs_(d, staffId, ref), 'text/calendar', 'neva-randevu-' + ref + '.ics');
  MailApp.sendEmail({
    to: d.email,
...
function createEvent_(d, staffId, ref) {
  ...
  var ev = getCalendar_().createEvent(
    ...
    start, end,
    {
      ...
      location: CONFIG.CLINIC_ADDRESS.tr
    }
  );
  return ev.getId();
}
...
function buildIcs_(d, staffId, ref) {
  ...
  return [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Neva Guzellik//Randevu//TR', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    'UID:' + ref + '@mizanbeslenme',
    'DTSTAMP:' + stamp(Date.now()),
    'DTSTART:' + stamp(start),
    'DTEND:' + stamp(end),
    'SUMMARY:' + escIcs('Neva Güzellik · ' + type[d.lang]),
    'DESCRIPTION:' + escIcs(STAFF[staffId].name + '\n' + ref + '\n' + CONFIG.CLINIC_PHONE),
    'LOCATION:' + escIcs(CONFIG.CLINIC_ADDRESS[d.lang]),
...
function makeRef_(existing) {
  ...
  return 'NV-' + String(Date.now()).slice(-5);
}
function hash_(s) {
